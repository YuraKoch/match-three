...
export class MatchThree {
  ...
  addTiles() {
    for (let row = 0; row < this.game.grid.length; row++) {
      for (let column = 0; column < this.game.grid[0].length; column++) {
        this.createTile(row, column, this.game.grid[row][column]);
      }
    }
  }
  ...
  handleTileClick = async (row, column) => {
    if (this.isGameBlocked) return;

    this.isGameBlocked = true;
    await this.processTileSwap(row, column);
    this.isGameBlocked = false;
  }

  async processTileSwap(row, column) {
    if (!this.selectedTile) {
      this.selectTile(row, column);
      return;
    }

    const selectedTilePosition = { row: this.selectedTile.row, column: this.selectedTile.column };
    const isSelectedNeighbours = this.game.isNeighbours(selectedTilePosition, { row, column });
    ...
    const firstTilePosition = { row: firstTile.row, column: firstTile.column };
    const secondTile = this.findTileBy(row, column);
    const secondTilePosition = { row: secondTile.row, column: secondTile.column };
    this.unselectTile();
    ...
    const swapResult = this.game.swapElements(firstTilePosition, secondTilePosition);
